"""
Chat Of Matrix - v1.0
Phase 1: Text Group Chat with Matrix Rain Effect (Render WebSocket backend)
"""
...
# ─────────────────────────────────────────────
#  SERVER CONFIG (Render WebSocket)
# ─────────────────────────────────────────────
SERVER_URL = "wss://phochatbackend.onrender.com"
...
GREEN_MID    = (0, 0.75, 0, 1)
GREEN_DIM    = (0, 0.45, 0, 1)
...
def green_label(text="", font_size=sp(14), bold=False, halign="left", markup=True, **kw):
    lbl = Label(text=text, font_size=font_size, bold=bold,
                color=GREEN_BRIGHT, halign=halign, markup=markup, **kw)
    lbl.bind(size=lambda l, s: setattr(l, 'text_size', s))
    return lbl

def matrix_button(text, on_press=None, bg=GREEN_DIM, fg=GREEN_BRIGHT,
                  size_hint_y=None, height=dp(44)):
    btn = Button(text=text, font_size=sp(13), bold=True,
...
def coming_soon_popup(feature):
    ...
    popup = Popup(title="// GELİŞTİRİLME AŞAMASINDA //",
                  title_color=GREEN_BRIGHT, content=content,
                  size_hint=(0.82, 0.48),
                  background_color=(0, 0.06, 0, 0.97),
                  separator_color=GREEN_MID, auto_dismiss=True)
...
class LoginScreen(Screen):
    def __init__(self, **kwargs):
        ...
        title = Label(text="[b][color=00FF00]C H A T   O F   M A T R I X[/color][/b]",
                      markup=True, font_size=sp(22),
                      size_hint_y=None, height=dp(50))
        subtitle = Label(text="[color=007300]// v1.0 — Encrypted Group Channel //[/color]",
        ...
        nick_lbl = green_label(text="[b]> NİCKNAME GİR:[/b]",
                               font_size=sp(13),
        ...
        self.status_lbl = Label(text="", markup=True,
                                font_size=sp(11), color=GREEN_MID,
                                size_hint_y=None, height=dp(22))
        conn_btn = matrix_button("[ MATRİSE GİR ]",
    ...
    def _do_connect(self, *_):
        ...
        if len(nick) > 20:
            ...
            return
        self.status_lbl.text = "[color=00BF00]> Sunucuya bağlanılıyor... (Render uyanıyorsa 30sn sürebilir)[/color]"
    ...
    def _on_connected(self, dt):
        app = App.get_running_app()
        try:
            app.sock.send(app.nickname)
        except Exception:
            pass
        self.status_lbl.text = "[color=00FF00]> Bağlantı kuruldu! Kanal açılıyor...[/color]"
        Clock.schedule_once(lambda dt: setattr(self.manager, "current", "chat"), 0.6)
...
class MessageBubble(BoxLayout):
    def __init__(self, sender, text, ts, is_own=False, is_system=False, **kw):
        super().__init__(orientation="vertical", size_hint_y=None,
                         padding=dp(6), spacing=dp(2), **kw)
        if is_system:
            lbl = Label(text=f"[color=007300][i]{text}[/i][/color]",
                        markup=True, font_size=sp(11),
        ...
        inner = BoxLayout(orientation="vertical", size_hint_x=0.78,
        ...
        body = Label(text=f"[color=00FF00]{text}[/color]",
...
class ChatScreen(Screen):
    def __init__(self, **kwargs):
        ...
        topbar = BoxLayout(size_hint_y=None, height=dp(52),
        ...
        for icon, feat in [("📷", "Fotoğraf Gönder"),
                           ...
                           ("📞", "Sesli Arama"),
                           ("🎥", "Görüntülü Arama")]:
            btn = Button(text=icon, font_size=sp(18),
                         size_hint=(None, None), size=(dp(36), dp(36)),
                         background_normal="", background_color=(0, 0.15, 0, 1))
            btn.bind(on_press=lambda b, f=feat: coming_soon_popup(f).open())
        ...
        self.msg_box = BoxLayout(orientation="vertical",
        ...
        send_btn = matrix_button("GÖNDER", on_press=self._send_msg,
                                 bg=(0, 0.28, 0, 1), fg=GREEN_BRIGHT,
    ...
    def on_enter(self):
        ...
        self.conn_bar.text = (
            f"[color=00FF00]> Bağlandı: phochatbackend.onrender.com[/color]")
    ...
    def _insert_bubble(self, sender, text, ts, is_own=False, is_system=False):
        bubble = MessageBubble(sender=sender, text=text, ts=ts,
